...
public class DownloadConnectionAdapter implements DownloadConnection, DownloadConnection.Connected {
    ...
    static final class RedirectHandler implements IRedirectHandler {
        ...
        @Override
        public void handleRedirect(DownloadConnection originalConnection, Connected originalConnected, Map<String, List<String>> headerProperties) throws IOException {
            if (!(originalConnection instanceof DownloadConnectionAdapter)) return;
            final String requestMethod = ((DownloadConnectionAdapter) originalConnection).requestMethod;
