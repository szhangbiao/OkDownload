...
public class CompatListenerAssist {
    ...
    public void fetchProgress(@NonNull DownloadTask task, long increaseBytes) {
        final DownloadTaskAdapter downloadTaskAdapter = FileDownloadUtils.findDownloadTaskAdapter(task);
        if (downloadTaskAdapter == null) return;
        downloadTaskAdapter.getProgressAssist().onProgress(downloadTaskAdapter, increaseBytes, callback);
    }
    ...
    void handleCanceled(@NonNull DownloadTaskAdapter downloadTaskAdapter) {
        callback.paused(downloadTaskAdapter, downloadTaskAdapter.getProgressAssist().getSofarBytes(), downloadTaskAdapter.getTotalBytesInLong());
    }
    ...
    void handleError(@NonNull DownloadTaskAdapter downloadTaskAdapter, @Nullable Exception realCause) {
        ...
        if (retryAssist != null && retryAssist.canRetry()) {
            ...
            callback.retry(downloadTaskAdapter, realCause, retryingTime, downloadTaskAdapter.getProgressAssist().getSofarBytes());
    ...
    public interface CompatListenerAssistCallback {
         ...
         */
        void connected(BaseDownloadTask task, String etag, boolean isContinue, long soFarBytes, long totalBytes);
         ...
         */
        void retry(BaseDownloadTask task, Throwable ex, int retryingTimes, long soFarBytes);
