...
public class FileDownloader {
     ...
     */
    public static void init(@NonNull Context context, @Nullable final FileDownloadHelper.OkHttpClientCustomMaker okHttpClientCustomMaker, int maxNetworkThreadCount) {
        setup(context);
    ...
    @Nullable
    public static OkDownload.Builder okDownloadBuilder(@NonNull Context context, @Nullable final FileDownloadHelper.OkHttpClientCustomMaker okHttpClientCustomMaker) {
    ...
    public boolean start(final FileDownloadListener listener, final boolean isSerial) {
        if (listener == null) {
            Util.w(TAG, "Tasks with the listener can't start, because the listener " + "provided is null: [null, " + isSerial + "]");
            return false;
        }
        ...
        final DownloadContext downloadContext = new DownloadContext.Builder(new DownloadContext.QueueSet(), downloadTasks)
                .setListener(new DownloadContextListener() {
                    ...
                    public void taskEnd(@NonNull DownloadContext context, @NonNull DownloadTask task, @NonNull EndCause cause, @Nullable Exception realCause, int remainCount) {
                        ...
                        final DownloadTaskAdapter downloadTaskAdapter = FileDownloadUtils.findDownloadTaskAdapter(task);
