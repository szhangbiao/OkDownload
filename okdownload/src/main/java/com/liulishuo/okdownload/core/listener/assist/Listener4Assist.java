...
public class Listener4Assist<T extends Listener4Assist.Listener4Model> implements ListenerAssist {
    ...
    public void infoReady(DownloadTask task, BreakpointInfo info, boolean fromBreakpoint) {
        ...
        if (assistExtend != null && assistExtend.dispatchInfoReady(task, info, fromBreakpoint, model)) {
            return;
        }
    ...
    public void fetchProgress(DownloadTask task, int blockIndex, long increaseBytes) {
        ...
        if (assistExtend != null && assistExtend.dispatchFetchProgress(task, blockIndex, increaseBytes, model)) {
            return;
        }
        if (callback != null) {
    ...
    public void fetchEnd(DownloadTask task, int blockIndex) {
        final Listener4Model model = modelHandler.getOrRecoverModel(task, task.getInfo());
        if (model == null) return;
        if (assistExtend != null && assistExtend.dispatchBlockEnd(task, blockIndex, model)) {
            return;
        }
        if (callback != null) {
    ...
    public synchronized void taskEnd(DownloadTask task, EndCause cause, @Nullable Exception realCause) {
        Listener4Model model = modelHandler.removeOrCreate(task, task.getInfo());
        if (assistExtend != null && assistExtend.dispatchTaskEnd(task, cause, realCause, model)) {
            return;
        }
    ...
    public interface AssistExtend {
        boolean dispatchInfoReady(DownloadTask task, @NonNull BreakpointInfo info, boolean fromBreakpoint, @NonNull Listener4Model model);

        boolean dispatchFetchProgress(@NonNull DownloadTask task, int blockIndex, long increaseBytes, @NonNull Listener4Model model);

        boolean dispatchBlockEnd(DownloadTask task, int blockIndex, Listener4Model model);

        boolean dispatchTaskEnd(DownloadTask task, EndCause cause, @Nullable Exception realCause, @NonNull Listener4Model model);
    }

    public interface Listener4Callback {

        void infoReady(DownloadTask task, @NonNull BreakpointInfo info, boolean fromBreakpoint, @NonNull Listener4Model model);
        ...
        void taskEnd(DownloadTask task, EndCause cause, @Nullable Exception realCause, @NonNull Listener4Model model);
    }
