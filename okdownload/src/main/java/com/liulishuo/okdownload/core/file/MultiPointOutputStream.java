...
public class MultiPointOutputStream {
    ...
    MultiPointOutputStream(@NonNull final DownloadTask task, @NonNull BreakpointInfo info, @NonNull DownloadStore store, @Nullable Runnable syncRunnable) {
        this.task = task;
    ...
    public MultiPointOutputStream(@NonNull DownloadTask task, @NonNull BreakpointInfo info, @NonNull DownloadStore store) {
    ...
    void runSyncDelayException() {
        try {
            ...
            syncException = e;
            Util.w(TAG, "Sync to breakpoint-store for task[" + task.getId() + "] " + "failed with cause: " + e);
        }
    }
